...
namespace WpfApps
{
    ...
    public partial class PreviewModel : Window
    {
        ...
        // LoadFamilyExEvent lfEx = null;
        // ExternalEvent lfEvent = null;  //加载族的外部事件

        //  PreviewExEvent prEx = null;
        // ExternalEvent prEvent = null; //预览族的外部事件
        public PreviewControl pc = null;
        DirectoryInfo dirInfo = null;
        private string familyFilePath = string.Empty;
        List<DBViewItem> dbViewsList = null;

        List<string> dlName = new List<string> { "精细", "中等", "粗略" };
        ...
        public PreviewModel(RApplication rapp, UIApplication uiapp)
        {
            ...
            //  lfEx = new LoadFamilyExEvent();
            // lfEvent = ExternalEvent.Create(lfEx);

            // prEx = new PreviewExEvent();
            // prEvent = ExternalEvent.Create(prEx);
        }
        ...
        private void FamilyTree_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            {
                if (dirInfo.GetFiles("*.rfa").Count() == 0) return;
        ...
        private void CreateFamilyItems(DirectoryInfo dirInfo, System.Windows.Controls.Control control)
        {
            foreach (FileInfo fi in dirInfo.GetFiles("*.rfa"))
            {
                ...
                {
                    ((System.Windows.Controls.TreeView)control).Items.Add(item);
                    continue;
                }
        ...
        private void UpLoadBTN_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                _openfiledialog.Filter = "族文件 (*.rfa)|*.rfa"; //过滤族文件
                ...
                string destFileName = _openfiledialog.FileName; //选择文件的完整路径
