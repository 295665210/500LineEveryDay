    ...
    class Cmd_BreakWire : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            while (true)
            {
                try
                {
                    var eleref = sel.PickObject(ObjectType.PointOnElement,
                        doc.GetSelectionFilter(m => true, n => n.GetElement(doc) is Wire));
                    var eleref1 = sel.PickObject(ObjectType.PointOnElement,
                        doc.GetSelectionFilter(m => true, n => n.ElementId == eleref.ElementId));
                    ...
                    var secondpo = secondLinefirst.DistanceTo(point) < secondLinefirst.DistanceTo(point1)
                        ? point
                        : point1;
